...
{
	int serverPortNumber, activity, max_fd, clientPortNumber, length;
	...
	while(1)
	{
		...
		if(FD_ISSET(fileno(stdin), &readfds))
		{
			...
			// send data to server
			if(fgets(dataToServer, LENGTH_ARRAY, stdin) == NULL)
			{
				perror("fgets()!");
				exit(EXIT_FAILURE);
			}
		...
		if(FD_ISSET(clientFd, &readfds))
		{
			...
			if(recvfrom(clientFd, dataFromServer, sizeof(dataFromServer), 0, NULL, NULL) == -1)
			{
				perror("recvfrom()!");
				exit(EXIT_FAILURE);
			}
